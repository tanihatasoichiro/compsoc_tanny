...
def generate_unani_votes(winner:int,number_voters: int,
                          number_candidates: int) -> List[Tuple[int, Tuple[int, ...]]]:
    candidate_list = list(range(number_candidates)).remove(winner)
    #いったんremove,insert使う
    #とりあえずrandomのみ対応
    votes = [random.sample(candidate_list, number_candidates-1).insert(0,winner) for _ in range(number_voters)]
    vote_strs = [int_list_to_str(vote) for vote in votes]
    vote_counts = Counter(vote_strs)
    ballots = [(count, tuple(map(int, vote.split(',')))) for vote, count in vote_counts.items()]
    return ballots
...
def get_profile_from_model(num_candidates: int, num_voters: int, voters_model,
...
def get_pairs_from_model(num_candidates: int, num_voters: int, voters_model, *args, **kwargs):
    """
