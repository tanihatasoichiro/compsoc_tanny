...
def ni_check(rules: list,all_result: list,rep,num_candidates: int,
                          num_voters: int,
                          topn: int,
                          voters_model: str,
    ...
    s="non-imposition"
    results=[[0 for _ in range(num_candidates)] for _ in rules]
    ...
    for _ in range(rep):
        ...
        for i in range(len(rules)):
            result.append(get_ranking(profile, rules[i], topn, verbose))
        #print(result)
        for i in range(len(rules)):
            if results[i][result[i][1][0]]==0:
                results[i][result[i][1][0]]=1
    n=all_result[0].index(s)
    #print(results)
    for i in range(len(rules)):
        all_result[i+1][n]=sum(results[i])==num_candidates
    return all_result
