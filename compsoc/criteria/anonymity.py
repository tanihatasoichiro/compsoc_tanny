...
def anonymity_check(rules: list,all_result: list,rep,num_candidates: int,
                          num_voters: int,
                          topn: int,
                          voters_model: str,
    ...
    s="anonymity"
    for _ in range(rep):
        #random.shuffle(profile.pairs)←これやるとSetにはsuffleは定義されてないよーって言われる。
        #profile.pairsが元々setであるため、
        #順番は関係ないのでそのままでもAnonymityは示されているとみなした。（いいのか。。。）
        g.num=random.choice(range(len(profile.pairs)))
        result=[]
        g.ano=True
        g.neu=False
        for rule in rules:
